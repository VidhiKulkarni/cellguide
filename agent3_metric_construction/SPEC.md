...
- **On the real n=199 benchmark, the accessibility gate reduces prediction quality** relative to sequence gates alone (F1 0.593 -> 0.431 — see `agent4_benchmarking/output/REPORT.md`'s "Does the accessibility gate earn its keep?" section), and the underlying ATAC measurement is itself unstable across replicates (`agent4_benchmarking/output/REPLICATE_SENSITIVITY_REPORT.md`). As currently validated, chromatin accessibility — this project's core differentiator — has not demonstrated predictive value; `recommended_score` is sequence-only for exactly this reason.
